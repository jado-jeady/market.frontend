    ...
    if (window.confirm('Are you sure you want to delete this user?')) {
      try {
        ...
        if (response.ok) {
          ...
          toast.sucess('User deleted successfully!');
        }
      } catch (error) {
        console.error('API call failed:', error);
        toast.error('Error deleting user (Demo mode)');
      }
    }
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 transition-opacity">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 transform transition-all">
            ...
            <div>
              ...
              <select value={newUser.role} onChange={(e) => setNewUser({ ...newUser, role: e.target.value })} className="w-full px-4 py-2 border text-gray-600 border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500">
