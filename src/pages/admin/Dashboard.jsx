...
import { 
  ...
  Users, ShoppingCart, DollarSign, Calendar, Clock,
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const [salesRes, productsRes, usersRes, categoriesRes, shiftsRes] = await Promise.all([
      ...
      const lowStockItems = allProducts.filter(p => p.stock_quantity <= (p.low_stock_threshold || 5));
      const allUsers = usersRes?.data || [];
      const allCategories = categoriesRes?.data || [];
      const allShifts = shiftsRes?.data || [];

      // Calculate total sales amount
      const totalSalesAmount = allSales.reduce((sum, sale) => sum + Number(sale.subtotal || 0), 0);
      
      // Calculate total stock quantity
      const totalStockQuantity = allProducts.reduce((sum, product) => sum + Number(product.stock_quantity || 0), 0);
      ...
      // Average order value
      const averageOrderValue = allSales.length > 0 ? totalSalesAmount / allSales.length : 0;
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        salesTrend.push({
          ...
          fullDate: dateStr,
          sales: daySales,
          transactions: allSales.filter(sale => sale.created_at?.startsWith(dateStr)).length
        });
      }
      ...
      const recentActivities = [
        ...
        ...allProducts.filter(p => p.stock_quantity <= (p.low_stock_threshold || 5)).slice(0, 3).map(p => ({
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-green-50 p-4 rounded-lg">
              ...
              <p className="text-xl font-bold text-gray-900">{formatCurrency(stats.monthlySales)}</p>
            ...
            <div className="bg-blue-50 p-4 rounded-lg">
              ...
              <p className="text-xl font-bold text-gray-900">{formatCurrency(stats.weeklySales)}</p>
            ...
            <div className="bg-purple-50 p-4 rounded-lg">
              ...
              <p className="text-xl font-bold text-gray-900">{formatNumber(stats.totalStock)}</p>
