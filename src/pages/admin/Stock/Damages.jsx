...
function ReportedByCell({ userId }) {
  ...
  useEffect(() => {
    ...
    getUserNameById(userId).then((res) => {
      if (mounted) setUsername(res?.data?.full_name ?? "—");
    });
    return () => {
      mounted = false;
    };
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-4 py-2.5 text-sm bg-white text-gray-900 border border-slate-200 rounded-xl outline-none
                      ...
                      >
                        <td className="px-4 py-3">
                          <span className="font-mono text-xs bg-slate-100 text-slate-600 font-semibold px-2 py-1 rounded-md">
                            {String(r.ref_number).slice(0, 8).toUpperCase()}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                        ...
                        <ReportedByCell userId={r.reported_by} />
                        ...
                        <td className="px-4 py-3 text-slate-600 text-xs">
                          <button onClick={() => handleApprove(r.id)}>
                            <CheckLine size={15} className="inline mr-1" />
                          </button>
                        </td>
                      </tr>
                    ))
