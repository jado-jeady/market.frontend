    ...
    (async () => {
      ...
      if (response.success) {
        ...
        await Promise.all(uniqueIds.map(async (id) => {
          ...
          nameMap[id] = userRes?.data?.full_name || userRes.username || "Unknown";
        }));
          ...
          <tbody className="divide-y text-gray-700">
            {shifts.map((shift) => (
              <tr key={shift.id} className="hover:bg-gray-50">
                ...
                <td className="px-4 py-3 text-right">{shift.opening_balance || "-"}</td>
                <td className="px-4 py-3 text-right">{shift.closing_balance || "-"}</td>
                <td className="px-4 py-3 text-right">{shift.total_sales || 0}</td>
                <td className="px-4 py-3 text-right">{shift.difference || 0}</td>
                <td className="px-4 py-3 text-center space-x-2">
                  {shift.status === "OPEN" && (
                    <button onClick={() => handleCloseShift(shift.id)} className="px-3 py-1 bg-red-600 text-white rounded">Close</button>
                  ) || shift.status === "CLOSED" && (
                    <p className="px-3 py-1 text-green-800 italic rounded">Closed</p>
                  )}
                </td>
              </tr>
            ))}
